...
p2 = ggplot(top_artists) + geom_histogram(aes(x=num_singles, y=cumsum(..count../sum(..count..))), bins=31) + ylab('Cummulative Density')
...
We encountered several problems; the first major problem with the data was fused words. Fused words was the situation when there exists no demarcation between the final word of a line and the first word of the next line, resulting in fused words.

Word clouds have formed an important part of the project and messy word data was one of the biggest problems we had. If we had more time would have liked to use an NLP package to tag out of vocabulary words to estimate the extent of the issue. We currently removed these words from the data.
...
All in all, this project turned out to be a really interesting one and it gave us a thourough knowledge of data visualization and the music industry. We learned about the evolution of the very music we listen to every day and saw how things changed!
